...
import numpy as np
import time
from model import myNetBaseUNetAtt, myNetSwinUNETRAtt
...
class Train():
    ...
    def netInit(self, mode='train'):
        in_channels = 3 if self.label == 'gtvctv' else 2
        out_channels = 2 if self.label == 'gtvctv' else 1
        if 'SwinUNETR' in self.netName:
            self.net = myNetSwinUNETRAtt(in_channels=in_channels, out_channels=out_channels, use_ram=self.use_ram)
        else:
            self.net = myNetBaseUNetAtt(in_channels=in_channels, out_channels=out_channels, use_ram=self.use_ram)
